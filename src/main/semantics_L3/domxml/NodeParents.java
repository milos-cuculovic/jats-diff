						...
						if (cOb.hasOp() || cOb.getChangement().equals("delete")
							...
							if (cOb.isFrom() || cOb.getChangement().equals("ref-delete")) {
								...
								if (cOb.hasAtA()) {
									...
									int nnA = Integer.parseInt(cOb.getNnBRef());
									Dnode dnodeorig = treeorig.getNode(nnA);
									if (dnodeorig.refDomNode.getNodeType() == Node.ELEMENT_NODE) {
										Element e = (Element) dnodeorig.refDomNode;
										if (!nCh.hasDepth()) {
		...
		for (NodeChanged nCh : modif1) {
			if (!nCh.isA()) {
				...
				if (n.getNodeName().equals("disp-formula") || n.getNodeName().equals("inline-formula")
						|| n.getNodeName().equals("contrib") || n.getNodeName().equals("ref")) {
					modif.remove(nCh);
					...
					while (!n.getNodeName().equals("article") && !n.getNodeName().equals("body")
						...
						if (noeuda.refDomNode.getNodeName().equals("disp-formula")
								...
								|| noeuda.refDomNode.getNodeName().equals("ref")
				...
				if (n.getNodeName().equals("disp-formula") || n.getNodeName().equals("inline-formula")
					...
					while (!n.getNodeName().equals("front") && !n.getNodeName().equals("body")
						...
						if (papB == null) {
							...
							noeudB = treem.getNode(papB);
						} else if(noeudB.refDomNode.getNodeName().equals("ref")
									|| noeudB.refDomNode.getNodeName().equals("inline-formula")
									|| noeudB.refDomNode.getNodeName().equals("disp-formula")
									|| noeudB.refDomNode.getNodeName().equals("contrib")|| noeudB.refDomNode.getNodeName().equals("article")){
						...
						if (n.getNodeName().equals("disp-formula") || n.getNodeName().equals("inline-formula")
							...
							if (noeudB.refDomNode.getNodeName().equals("ref")
									|| noeudB.refDomNode.getNodeName().equals("inline-formula")
									|| noeudB.refDomNode.getNodeName().equals("disp-formula")
									|| noeudB.refDomNode.getNodeName().equals("contrib")) {
								nCh.setNodenumberB(papB);
								co.setNodenumB(Integer.toString(papB));
							} else if (noeudB.refDomNode.getParentNode().getNodeName().equals("ref")
									|| noeudB.refDomNode.getParentNode().getNodeName().equals("inline-formula")
							...
							} else if (n.getNodeName().equals("contrib")) {
								co.setChangement("contrib-edit");
							} else {
								co.setChangement("formula-edit");
							}
